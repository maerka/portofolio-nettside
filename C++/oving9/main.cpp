        ...
        while (window.pollEvent(event)) {
            ...
            case sf::Event::MouseButtonPressed:
                if (event.mouseButton.button == sf::Mouse::Left && !game->isGameOver()) {
					...
					//cout << "\tfoer game->openTile(row, col);" << endl;
                    game->openTile(row, col);
					//cout << "\tetter game->openTile(row, col);" << endl;

                    if (game->isGameOver()) {
                        cout << "\nKABOOOM \nTrykk ESC eller Q for å avslutte, eller MELLOMROM for å starte på nytt" << endl;
                    }
					else if (game->allMinesFound()) {
						cout << "GRATULERER! Alle minene er detektert. Det er trygt å sende inn troppene." << endl;
					}
                }
                break;
            }
